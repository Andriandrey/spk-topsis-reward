...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  ...
  if ($Result1) {
  	echo "<script>
	document.location = '?page=alternatif/view';
	</script>";
  }
}
...
mysql_select_db($database_koneksi, $koneksi);
$query_rs_cek = sprintf("SELECT alternatif_id FROM tb_bobot WHERE alternatif_id = %s", GetSQLValueString($colname_rs_alternatif, "int"));
	...
	if ($totalRows_rs_cek == 0) {
		 do {
		  $insertSQL = sprintf("INSERT INTO tb_bobot (kriteria_id, alternatif_id) VALUES (%s, %s)",
							   ...
							   GetSQLValueString($colname_rs_alternatif, "int"));
        ...
        <tbody>
		  ...
		  //MENGAMBIL NILAI BOBOT
			...
			$query_rs_bobot = sprintf("SELECT nilai_bobot FROM tb_bobot WHERE alternatif_id = %s AND kriteria_id = %s", 
										...
										GetSQLValueString($row_rs_kriteria['id_kriteria'], "int"));
