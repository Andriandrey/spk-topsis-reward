...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
    ...
    mysql_select_db($database_koneksi, $koneksi);
    $query_rs_cek = sprintf(
        "SELECT Login FROM tb_master WHERE Login = %s",
        GetSQLValueString($_POST['Login'], "text")
    );
    $rs_cek = mysql_query($query_rs_cek, $koneksi) or die(mysql_error());
    $totalRows_rs_cek = mysql_num_rows($rs_cek);
    ...
    if ($totalRows_rs_cek == 0) {
        $insertSQL = sprintf(
            "INSERT INTO tb_admin (Login, Password, nama_admin, gender_admin, address_admin, email_admin, hp_admin, photo_admin, `Level`, cb, key_admin) VALUES (%s, PASSWORD(%s), %s, %s, %s, %s, %s, %s, %s, %s, %s)",
            GetSQLValueString($_POST['Login'], "text"),
            ...
            GetSQLValueString($_POST['photo_admin'], "text"),
        ...
        <table width="411" height="364">
            ...
            <tr valign="baseline">
                ...
                <td valign="baseline">
                    <table>
                        <tr>
                            <td><input type="radio" name="gender_admin" value="L" <?php if (!(strcmp("L", "L"))) {
                                                                                        echo "CHECKED";
                                                                                    } ?> />
                                Laki-laki</td>
                        </tr>
                        <tr>
                            <td><input type="radio" name="gender_admin" value="P" <?php if (!(strcmp("L", "P"))) {
                                                                                        echo "CHECKED";
                                                                                    } ?> />
                                Perempuan</td>
                        </tr>
                ...
                do { ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <a onclick="return confirm('Hapus Admin <?= $row_rs_admin['id_admin']; ?>?');" href="?page=delete/hapus&id_admin=<?= $row_rs_admin['id_admin']; ?>" class="btn btn-xs btn-danger" title="Hapus"><span class="fa fa-trash"></span></a>
                        </td>
                    </tr>
