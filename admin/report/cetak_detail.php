...
GetSQLValueString($koneksi, $colname_rs_jadwal, "text"),
GetSQLValueString($koneksi, $colname_rs_kelas, "text"));
$rs_detailhasilESSAY = mysqli_query($koneksi, $query_rs_detailhasilESSAY) or die(mysqli_error($koneksi));
$row_rs_detailhasilESSAY = mysqli_fetch_assoc($rs_detailhasilESSAY);
$totalRows_rs_detailhasilESSAY = mysqli_num_rows($rs_detailhasilESSAY);
    ...
    <td><?php echo $row_rs_detailhasilESSAY['mulai_jadwal']; ?> s/d <?php echo $row_rs_detailhasilESSAY['akhir_jadwal']; ?></td>
                    ...
                    <thead>
					  ...
					  $query_rs_nilai = "SELECT kodepeserta_ujian, SUM(nilai_ujian = 'B') as Benar, SUM(nilai_ujian = 'S') as Salah FROM `tb_ujian` INNER JOIN tb_peserta ON Login = kodepeserta_ujian INNER JOIN tb_kelas ON id_kelas = kelas_peserta INNER JOIN tb_soal ON id_soal = soal_ujian INNER JOIN tb_jadwal ON kode_jadwal = kodejadwal_ujian WHERE kodejadwal_ujian = '".$row_rs_detailhasilESSAY['kodejadwal_ujian']."' AND kodepeserta_ujian = '".$row_rs_detailhasilESSAY['kodepeserta_ujian']."' ORDER BY `tb_kelas`.`nama_kelas` ASC";
						$rs_nilai = mysqli_query($koneksi, $query_rs_nilai) or die(mysqli_error($koneksi));
						$row_rs_nilai = mysqli_fetch_assoc($rs_nilai);
						$totalRows_rs_nilai = mysqli_num_rows($rs_nilai);
					  ?>
                        <tr>
                        ...
                        <?php } while ($row_rs_detailhasilESSAY = mysqli_fetch_assoc($rs_detailhasilESSAY)); ?>
                        </tbody>
                    </table> 
