...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  if (empty($_POST['Password'])) {
    $updateSQL = sprintf(
      "UPDATE tb_admin SET  nama_admin=%s, gender_admin=%s, address_admin=%s, email_admin=%s, hp_admin=%s, `Level`=%s, active_admin=%s, key_admin=%s, ut=%s WHERE id_admin=%s",
      ...
      GetSQLValueString($_POST['ut'], "date"),
    ...
    $updateSQL = sprintf(
      "UPDATE tb_admin SET Password=PASSWORD(%s), nama_admin=%s, gender_admin=%s, address_admin=%s, email_admin=%s, hp_admin=%s, `Level`=%s, active_admin=%s, key_admin=%s, ut=%s WHERE id_admin=%s",
...
$rs_editadmin = mysql_query($query_rs_editadmin, $koneksi) or die(mysql_error());
$row_rs_editadmin = mysql_fetch_assoc($rs_editadmin);
$totalRows_rs_editadmin = mysql_num_rows($rs_editadmin);
...
mysql_select_db($database_koneksi, $koneksi);
$query_rs_admin = sprintf("SELECT * FROM tb_admin WHERE id_admin = %s AND active_admin = 'Y'", GetSQLValueString($colname_rs_admin, "int"));
$rs_admin = mysql_query($query_rs_admin, $koneksi) or die(mysql_error());
...
$rs_level = mysql_query($query_rs_level, $koneksi) or die(mysql_error());
      ...
      <table width="358" height="373">
        ...
        <tr valign="baseline">
          ...
          <td valign="baseline">
            <table>
              <tr>
                <td><input type="radio" name="gender_admin" value="L" <?php if (!(strcmp(htmlentities($row_rs_admin['gender_admin'], ENT_COMPAT, 'utf-8'), "L"))) {
                                                                        echo "CHECKED";
                                                                      } ?> />
                  Laki-laki</td>
              </tr>
              <tr>
                <td><input type="radio" name="gender_admin" value="P" <?php if (!(strcmp(htmlentities($row_rs_admin['gender_admin'], ENT_COMPAT, 'utf-8'), "P"))) {
                                                                        echo "CHECKED";
                                                                      } ?> />
                  Perempuan</td>
              </tr>
        ...
        <tr valign="baseline">
          ...
          <td><select class="form-control input-sm" name="Level" class="form-control input-sm">
              ...
              do {
              ?>
                <option value="<?php echo $row_rs_level['id_level'] ?>" <?php if (!(strcmp($row_rs_level['id_level'], htmlentities($row_rs_admin['Level'], ENT_COMPAT, 'utf-8')))) {
                                                                          echo "selected=\"selected\"";
                                                                        } ?>><?php echo $row_rs_level['nama_level'] ?></option>
              <?php
        ...
        <tr valign="baseline">
          ...
          <td><select class="form-control input-sm" name="active_admin" class="form-control input-sm">
              <option value="Y" <?php if (!(strcmp("Y", htmlentities($row_rs_admin['active_admin'], ENT_COMPAT, 'utf-8')))) {
                                  echo "SELECTED";
                                } ?>>Aktif</option>
              <option value="N" <?php if (!(strcmp("N", htmlentities($row_rs_admin['active_admin'], ENT_COMPAT, 'utf-8')))) {
                                  echo "SELECTED";
                                } ?>>Blok</option>
            </select> </td>
        </tr>
        ...
        do { ?>
          <tr>
            ...
            <td>
              ...
              <a onclick="return confirm('Hapus Admin <?= $row_rs_editadmin['id_admin']; ?>?');" href="?page=delete/hapus&id_admin=<?= $row_rs_editadmin['id_admin']; ?>" class="btn btn-xs btn-danger" title="Hapus"><span class="fa fa-trash"></span></a>
            </td>
          </tr>
        <?php } while ($row_rs_editadmin = mysql_fetch_assoc($rs_editadmin)); ?>
      </tbody>
    </table>
  </div>
</div>