...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE tb_bstk SET no_bstk=%s, bon_bstk=%s, donoI_bstk=%s, donoII_bstk=%s, tanggal_bstk=%s, type_bstk=%s, mesin_bstk=%s, ket_bstk=%s, active_bstk=%s, ub_bstk=%s, ut_bstk=%s WHERE id_bstk=%s",
                       GetSQLValueString($_POST['no_bstk'], "text"),
                       GetSQLValueString($_POST['bon_bstk'], "text"),
                       GetSQLValueString($_POST['donoI_bstk'], "text"),
                       GetSQLValueString($_POST['donoII_bstk'], "text"),
                       GetSQLValueString($_POST['tanggal_bstk'], "date"),
                       GetSQLValueString($_POST['type_bstk'], "int"),
                       GetSQLValueString($_POST['mesin_bstk'], "int"),
                       GetSQLValueString($_POST['ket_bstk'], "text"),
                       GetSQLValueString($_POST['active_bstk'], "text"),
                       GetSQLValueString($ID, "int"),
                       GetSQLValueString($today, "date"),
                       GetSQLValueString($_POST['id_bstk'], "int"));
...
mysql_select_db($database_koneksi, $koneksi);
$query_rs_bstk = sprintf("SELECT * FROM tb_bstk WHERE id_bstk = %s", GetSQLValueString($colname_rs_bstk, "int"));
$rs_bstk = mysql_query($query_rs_bstk, $koneksi) or die(mysql_error());
$row_rs_bstk = mysql_fetch_assoc($rs_bstk);
$totalRows_rs_bstk = mysql_num_rows($rs_bstk);
?>
