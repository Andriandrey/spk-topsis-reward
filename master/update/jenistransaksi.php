...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE tb_jenistransaksi SET kode_jenistransaksi=%s, nama_jenistransaksi=%s, active_jenistransaksi=%s WHERE id_jenistransaksi=%s",
                       GetSQLValueString($_POST['kode_jenistransaksi'], "text"),
                       GetSQLValueString($_POST['nama_jenistransaksi'], "text"),
                       GetSQLValueString($_POST['active_jenistransaksi'], "text"),
                       GetSQLValueString($_POST['id_jenistransaksi'], "int"));
...
mysql_select_db($database_koneksi, $koneksi);
$query_rs_jenistransaksi = sprintf("SELECT * FROM tb_jenistransaksi WHERE id_jenistransaksi = %s", GetSQLValueString($colname_rs_jenistransaksi, "int"));
$rs_jenistransaksi = mysql_query($query_rs_jenistransaksi, $koneksi) or die(mysql_error());
$row_rs_jenistransaksi = mysql_fetch_assoc($rs_jenistransaksi);
$totalRows_rs_jenistransaksi = mysql_num_rows($rs_jenistransaksi);
?>
