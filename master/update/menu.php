...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE tb_menu SET nourut_menu=%s, link_menu=%s, icon_menu=%s, text_menu=%s, color_menu=%s, label_menu=%s, level_menu=%s WHERE id_menu=%s",
                       GetSQLValueString($_POST['nourut_menu'], "int"),
                       GetSQLValueString($_POST['link_menu'], "text"),
                       GetSQLValueString($_POST['icon_menu'], "text"),
                       GetSQLValueString($_POST['text_menu'], "text"),
                       GetSQLValueString($_POST['color_menu'], "text"),
                       GetSQLValueString($_POST['label_menu'], "text"),
                       GetSQLValueString($_POST['level_menu'], "int"),
...
mysql_select_db($database_koneksi, $koneksi);
$query_rs_editmenu = sprintf("SELECT * FROM tb_menu WHERE id_menu = %s", GetSQLValueString($colname_rs_editmenu, "int"));
$rs_editmenu = mysql_query($query_rs_editmenu, $koneksi) or die(mysql_error());
$row_rs_editmenu = mysql_fetch_assoc($rs_editmenu);
$totalRows_rs_editmenu = mysql_num_rows($rs_editmenu);

mysql_select_db($database_koneksi, $koneksi);
$query_rs_menu = "SELECT * FROM tb_menu ORDER BY nourut_menu ASC";
$rs_menu = mysql_query($query_rs_menu, $koneksi) or die(mysql_error());
