...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE tb_operator SET Login=%s, Password=%s, `Level`=%s, nama_operator=%s, gender_operator=%s, alamat_operator=%s, email_operator=%s, nohp_operator=%s, photo_operator=%s, ket_operator=%s, key_operator=%s, ut_operator=%s, ub_operator=%s, active_operator=%s WHERE id_operator=%s",
                       ...
                       GetSQLValueString($_POST['nama_operator'], "text"),
                       GetSQLValueString($_POST['gender_operator'], "text"),
                       GetSQLValueString($_POST['alamat_operator'], "text"),
                       GetSQLValueString($_POST['email_operator'], "text"),
                       GetSQLValueString($_POST['nohp_operator'], "text"),
                       GetSQLValueString($_POST['photo_operator'], "text"),
                       GetSQLValueString($_POST['ket_operator'], "text"),
                       GetSQLValueString($_POST['key_operator'], "text"),
                       GetSQLValueString($_POST['ut_operator'], "date"),
                       GetSQLValueString($_POST['ub_operator'], "int"),
                       GetSQLValueString($_POST['active_operator'], "text"),
                       GetSQLValueString($_POST['id_operator'], "int"));
...
mysql_select_db($database_koneksi, $koneksi);
$query_rs_operator = sprintf("SELECT * FROM tb_operator WHERE id_operator = %s", GetSQLValueString($colname_rs_operator, "int"));
$rs_operator = mysql_query($query_rs_operator, $koneksi) or die(mysql_error());
$row_rs_operator = mysql_fetch_assoc($rs_operator);
$totalRows_rs_operator = mysql_num_rows($rs_operator);
?>
