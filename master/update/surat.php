...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE tb_surat SET tanggal_surat=%s, jenis_surat=%s, no_surat=%s, supir_surat=%s, penerima_surat=%s, ket_surat=%s, active_surat=%s WHERE id_surat=%s",
                       GetSQLValueString($_POST['tanggal_surat'], "date"),
                       GetSQLValueString($_POST['jenis_surat'], "text"),
                       GetSQLValueString($_POST['no_surat'], "text"),
                       GetSQLValueString($_POST['supir_surat'], "int"),
                       GetSQLValueString($_POST['penerima_surat'], "text"),
                       GetSQLValueString($_POST['ket_surat'], "text"),
                       GetSQLValueString($_POST['active_surat'], "text"),
                       GetSQLValueString($_POST['id_surat'], "int"));
...
mysql_select_db($database_koneksi, $koneksi);
$query_rs_surat = sprintf("SELECT * FROM tb_surat WHERE id_surat = %s", GetSQLValueString($colname_rs_surat, "int"));
$rs_surat = mysql_query($query_rs_surat, $koneksi) or die(mysql_error());
$row_rs_surat = mysql_fetch_assoc($rs_surat);
$totalRows_rs_surat = mysql_num_rows($rs_surat);
?>
