...
//cek email
	$x = GetSQLValueString($_POST['kode'], "text");
	$y = GetSQLValueString($_POST['a'], "text");

	  if ($x == $y) {
	       mysql_select_db($database_koneksi, $koneksi);
		  $sql = sprintf("SELECT email_member FROM tb_member WHERE email_member = %s", GetSQLValueString($_POST['email'], "text"));
  ...
  $insertSQL = sprintf("INSERT INTO tb_member (firstname_member, middlename_member, lastname_member, email_member, passwd_member, gender_member, contact_member, country_member, active_member, key_member, ta_member, date_member, updateby_member) VALUES (%s, %s, %s, %s, PASSWORD(%s), %s, %s, %s, %s, %s, %s, %s, %s)",
                       ...
                       GetSQLValueString($_POST['mid_name'], "text"),
  ...
  if ($Result1){
		...
		if (isset($_POST['email'])) {
		  ...
		  if ($loginFoundUser) {
		   ...
		   //$loginStrGroup  = mysql_result($koneksi, $LoginRS,0,'Level');
